    ...
    bindMultiPickerColumnChange(e) {
      ...
      if (newIndexArr[0] !== self.data.areaIndex[0]) {
        ...
        base.http.httpPost({
          ...
          data: { pid: self.data.provinceList[newIndexArr[0]].id },
          success(res) {
      ...
      if (newIndexArr[1] !== self.data.areaIndex[1]) {
        ...
        base.http.httpPost({
          url: "userAddress/getProvinces",
          data: { pid: self.data.cityList[newIndexArr[1]].id },
          success(res) {
            let areaList = res.data;
            areaList.map((item, index) => {
              item.name = item.cname;
    ...
    httpGetCertifyInfo(pid) {
      ...
      base.http.httpPost({
        ...
        success(result) {
          ...
          base.http.httpPost({
            url: "userAddress/getProvinces",
            data: { pid: result.data[0].id },
            success(res) {
              let areaList1 = res.data;
              areaList1.map((item, index) => {
                item.name = item.cname;
                delete item.cname
              })
              _that.setData({
